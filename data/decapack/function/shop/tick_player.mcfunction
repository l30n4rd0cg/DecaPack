...
#$execute unless entity @e[type=chest_minecart, nbt={UUID: [I; $(0), $(1), $(2), $(3)]}] run summon chest_minecart ~ ~ ~ {UUID: [I; $(0), $(1), $(2), $(3)]}

# # Check if player's in the shop menu. If he isn't, return. [REPLACED: now it's in decapack:shop/tick.mcfunction]
...
#execute store success score @s tmp1 run data modify storage decapack:tmp_command value set from block ~ ~2 ~ Items[{Slot: 26b, components: {"minecraft:custom_data": {"has_command": 1b}}}]
...
#execute if score @s tmp1 matches 1 run say Executing command.
#execute if score @s tmp1 matches 1 run tellraw @s [{text: "Executing "}, {storage: "decapack:tmp_command", nbt: "command"}]
...
#execute if score @s tmp1 matches 1 run tellraw @s ["[decapack:shop/tick_player: 35] decapack:tmp_args.slot = ", {storage: "decapack:tmp_args", nbt: "slot"}]
#execute if score @s tmp1 matches 1 run tellraw @s ["[decapack:shop/tick_player: 36] decapack:tmp_args.shop_index = ", {storage: "decapack:tmp_args", nbt: "shop_index"}]
